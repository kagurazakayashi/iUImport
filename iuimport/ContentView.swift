        ...
        NavigationView {
            List($files, id: \.path) { $file in  // 檔案列表
                HStack {
                    ...
                    let extName: String = mgr.getExtension(
                        fromPath: nowFile.path)  // 取得副檔名
                    ...
                    VStack(alignment: .leading) {  // 檔案資訊
                        ...
                        HStack {
                            let index = mgr.indexFileInfoModel(
                                fileInfos: files, path: nowFile.path)
            ...
            .toolbar {  // 工具列
                Button(action: {
                    UIApplication.shared.open(
                        URL(
                            string:
                                "https://github.com/kagurazakayashi/iuimport/blob/main/README.md"
                        )!)
                }) {
