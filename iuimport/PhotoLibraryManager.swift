...
class PhotoLibraryManager {
  ...
  func hasPhotoWritePermission(view: ContentView) -> Bool {  // 檢查是否擁有照片寫入權限
    let status: PHAuthorizationStatus = PHPhotoLibrary.authorizationStatus()  // 取得授權狀態
    switch status {
    case .authorized:
      return true  // 已授權
    case .notDetermined:
  ...
  func saveToPhotoLibrary(view: ContentView) {
    ...
    if permission == false {  // 沒有權限的情況
      if fileCount > 0 {
        ...
        view.savedStatusStr = Array(
          repeating: String(localized: "No album write permission!"), count: fileCount)
        view.files = files
      }
    }
    ...
    view.savedStatusStr = Array(
      repeating: String(localized: "Saving to album ..."), count: fileCount)
    view.files = files
  ...
  func saveOnce(view: ContentView) {
    ...
    if iconName == "photo" || iconName == "video" {
      ...
      view.photoLibrary.performChanges(
        isVideo
          ? { PHAssetChangeRequest.creationRequestForAssetFromVideo(atFileURL: url) }  // 儲存影片
          : { PHAssetChangeRequest.creationRequestForAssetFromImage(atFileURL: url) },  // 儲存圖片
